	...
	if(LPC_TIM1->IR&((0x1<<0))){		//Interrupt MR0
		...
		if(!(0 <= ((LPC_TIM1->TC)-(LPC_TIM2->TC)) && ((LPC_TIM1->TC)-(LPC_TIM2->TC)) <= 2)){ //Comprobacion de que uno va mas deprisa
			if((LPC_TIM1->TC) > (LPC_TIM2->TC) ){ // Motor 1 mas rapido
				if((LPC_PWM1->MR4*gain*1.0001)<LPC_PWM1->MR0){ // Comprobacion de que el tiempo en alto es menor que el periodo
					gain=gain*1.0001; // Incremento de la ganancia del 2
					calib(gain,velocidad); // Funcion calibracion
					if((LPC_PWM1->MR4*gain*1.0001)>LPC_PWM1->MR0){
						gain=gain*0.9999;
						calib(gain,velocidad);
					}				
				}
			}
		}
  ...
  Fc_bluetooth_communication();
	...
	while(1){
		if(flag==1){
			...
			default:
					LPC_GPIO1->FIOPIN=(LPC_GPIO1->FIOPIN&~((0x3)|(0x3<<16)));
			break;
